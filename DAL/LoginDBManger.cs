...
namespace DAL
{
    public class LoginDBManger
    {
        ...
        public static bool adminlogin(Admin c)
        {
            bool status = false;
            try
            {
                using (MySqlConnection con = new MySqlConnection(connString))
                {
                    ...
                    string query = "select * from admin_register where password = @upass and email = @uemail";

                    MySqlCommand cmd = new MySqlCommand(query, con);
                    cmd.Parameters.Add(new MySqlParameter("@uemail", c.email));
